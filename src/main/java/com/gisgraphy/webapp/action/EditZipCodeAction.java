 ...
 */
public class EditZipCodeAction extends BaseAction implements Preparable {

    private static final long serialVersionUID = -7493269463137268011L;

    private static Logger logger = LoggerFactory.getLogger(EditZipCodeAction.class);
	...
	if (zipCode != null) {
	    if (zipCode.getGisFeature()!=null){
		setGisFeatureId(zipCode.getGisFeature().getFeatureId());
	    }
	    GisFeature gisfeature = gisFeatureDao.getByFeatureId(getGisFeatureId());
	    if (gisfeature == null) {
		addFieldError("gisfeatureId", "no gisfeature to be link to the zipCode");
		return ERROR;
	    }
		...
		if (zipCode.getId() != null) {
		    ...
		    startTransaction();
		    try {
			gisfeature.getZipCodes().remove(zipCode);
			gisfeature.addZipCode(zipCode);
			gisFeatureDao.save(gisfeature);
		    } catch (Exception e) {
			rollbackTransaction();
			errorMessage = "could not save zipCode " + e.getMessage();
	    ...
	    errorMessage = "There is no zipCode to save";
	    logger.error(errorMessage);
	    return ERROR;
	}
    }
	...
	if (zipCode != null) {
		...
		addFieldError("code", internationalisationService.getString("errors.required", new String[] { "code" }));
	    }
	}
    }
	...
	if (zipCode != null) {
	    GisFeature feature = gisFeatureDao.getByFeatureId(zipCode.getGisFeature().getFeatureId());
	    feature.getZipCodes().remove(zipCode);
	   // alternateName.setGisFeature(null);
		...
		zipCodeDao.remove(zipCode);
		gisFeatureDao.flushAndClear();
	    } catch (Exception e) {
		logger.error("Can not delete zipcode : " + e.getMessage(), e);
		stackTrace = StringHelper.getStackTraceAsString(e);
		rollbackTransaction();
		return ERROR;
	    }
